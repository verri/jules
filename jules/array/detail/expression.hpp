...
binary_expr_ndarray<LhsIt, RhsIt, Op, N>::binary_expr_ndarray(const LhsIt& lhs_begin, const LhsIt& lhs_end,
                                                              const RhsIt& rhs_begin, const RhsIt& rhs_end, const Op& op,
                                                              const extent_t& extents)
    : lhs_begin_{lhs_begin}, lhs_end_{lhs_end}, rhs_begin_{rhs_begin}, rhs_end_{rhs_end}, op_{op}, extents_(extents)
{
}
...
template <typename It, typename Op, std::size_t N> auto unary_expr_ndarray<It, Op, N>::iterator::operator++(int) -> iterator
{
    auto c = *this;
    ++(*this);
    return c;
...
unary_expr_ndarray<It, Op, N>::unary_expr_ndarray(const It& it_begin, const It& it_end, const Op& op, const extent_t& extents)
...
binary_expr_ndarray<LhsI, RhsI, F, M> make_expr_ndarray(const LhsI& lhs_begin, const LhsI& lhs_end, const RhsI& rhs_begin,
                                                        const RhsI& rhs_end, const F& op,
