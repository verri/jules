...
struct array_apply
{
  ...
  {
    if constexpr (std::same_as<std::decay_t<decltype(op(std::declval<T>()))>, T>) {
...
#define UNARY_OPERATION(OP__, FUNCTOR__)                                                                                         \
  template <common_array Array> auto operator OP__(const Array& operand) { return ::jules::apply(operand, FUNCTOR__{}); }
